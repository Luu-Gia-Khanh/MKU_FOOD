                ...
                <div class="pd-20 card-box height-100-p">
                    <div class="profile-photo">
                        <img src="{{ asset('public/upload/' . $customer_info->customer_avt) }}" alt=""
                            style="border: 1px solid #e9ecef; width:160px; height:160px;" class="avatar-photo">
                        ...
                        <div class="tab-pane fade show active" role="tabpanel">
                            <div class="pd-10 content_filter_order_customer">
                                @if (count($all_order) > 0)
                                    <div class="row"
                                        style="margin-top: 15px; margin-bottom: -15px; padding-left: 10px;">
                                        <div class="col-sm-12 col-md-6 d-flex">
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-left" style="">
                                                        <a class="dropdown-item" href="#" data-toggle="modal"
                                                            data-target="#Modal_filter_order_customer_follow_price">Giá</a>
                                                        <a class="dropdown-item" href="#" data-toggle="modal"
                                                            data-target="#Modal_filter_order_customer_follow_date">Ngày mua
                                                            hàng</a>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="content_print_pdf_product ml-10">
                                                <form
                                                    action="{{ URL::to('admin/print_pdf_order_customer/' . $customer->customer_id) }}"
                                                    method="post">
                                                    @csrf
                                                    {{-- type filter --}}
                                                    ...
                                                    <button type="submit" class="btn btn-secondary">
                                                        Xuất
                                                        <img src="{{ asset('public/upload/pdf1.svg') }}"
                                                                        ...
                                                                        @foreach ($status_order as $status)
                                                                            @if ($status->status_id == $order_detail_status->status_id)
                                                                                @if ($order_detail_status->status_id == 1)
                                                                                    <span
                                                                                        class="cus_status_order_customer badge badge-warning">{{ $status->status_name }}</span>
                                                                                @elseif($order_detail_status->status_id
                                                                                    == 2)
                                                                                    <span
                                                                                        class="cus_status_order_customer badge badge-info">{{ $status->status_name }}</span>
                                                                                @elseif($order_detail_status->status_id
                                                                                    == 3)
                                                                                    <span
                                                                                        class="cus_status_order_customer badge"
                                                                                        style="background-color:rgb(0, 180, 137); color: white;">{{ $status->status_name }}</span>
                                                                                @elseif($order_detail_status->status_id
                                                                                    == 4)
                                                                                    <span
                                                                                        class="cus_status_order_customer badge badge-success">{{ $status->status_name }}</span>
                                                                                @elseif($order_detail_status->status_id
                                                                                    == 5)
                                                                                    <span
                                                                                        class="cus_status_order_customer badge badge-danger">{{ $status->status_name }}</span>
                                                                                @endif
                                                                            @endif
                                                                        @endforeach
                                                                    @endif
                                                                        ...
                                                                        <div class="cus_bottom_order_customer">
                                                                            @foreach ($trans as $tr)
                                                                                @if ($order->trans_id == $tr->trans_id)
                                                                                    <div
                                                                                        class="cus_bottom_order_customer--info">
                                                                                        <span
                                                                                            class="cus_bottom_order_customer--text">Người
                                                                                            nhận</span>
                                                                                    ...
                                                                                    <div
                                                                                        class="cus_bottom_order_customer--info">
                                                                                        <span
                                                                                            class="cus_bottom_order_customer--text">Số
                                                                                            điện thoại</span>
                                                                                    ...
                                                                                    <div
                                                                                        class="cus_bottom_order_customer--info">
                                                                                        <span
                                                                                            class="cus_bottom_order_customer--text">Địa
                                                                                            chỉ giao hàng</span>
                                                                    ...
                                                                    @if ($order_item->order_id == $order->order_id)
                                                                        <tr>
                                                                            <td class="table-plus sorting_1" tabindex="0">
                                                                                <div
                                                                                    class="name-avatar d-flex align-items-center">
                                                                                    @foreach ($all_product as $product)
                                                                                        @if ($order_item->product_id == $product->product_id)
                                                                                            <div
                                                                                                class="avatar mr-2 flex-shrink-0">
                                                                        ...
                                                                        <div class="cus_pay_order_customer">
                                                                            @foreach ($payment_method as $pay)
                                                                                @if ($pay->payment_id == $order->payment_id)
                                                                                    <span><img
                                                                                            src="{{ asset('public/upload/dollar.png') }}"
                                                                                            alt=""
