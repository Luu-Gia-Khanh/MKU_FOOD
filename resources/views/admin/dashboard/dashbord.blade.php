...
@section('container')
                ...
                <div class="card-box mb-30" style="position: relative;">
                    <div class="d-flex flex-wrap justify-content-between pl-20 pt-20 pr-20">
                        <div class="h6" id="title_chart_culumn">
                            Đồ Thị Biểu Diễn Đơn Hàng MKU FOOD {{ $year }}
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control form-control-sm selectpicker"
                                    id="select_year_filter_order_dashboard">
                ...
                <div class="card-box mb-30" style="position: relative;">
                    <div class="d-flex flex-wrap justify-content-between pd-20">
                        <div class="h6" id="title_chart_area">
                            Đồ Thị Biểu Diễn Doanh Thu MKU FOOD {{ $year }}
                        </div>
                        <div class="form-group">
                            <select class="form-control form-control-sm selectpicker"
                                id="select_year_filter_revenue_dashboard">
                                @for($i = $current_year; $i >= $current_year - 5; $i--)
                                    <option value="{{ $i }}">Năm {{ $i }}</option>
                                @endfor
                            </select>
                                    ...
                                    <div class="name-avatar d-flex align-items-center pr-2">
                                        <div class="avatar mr-2 flex-shrink-0">
                                            <img src="{{ asset('public/upload/'.$customer->customer_avt) }}"
                                                class="border-radius-100 box-shadow" style="width: 50px; height: 50px" alt="">
                                        </div>
                                        <div class="txt">
                                            <span class="badge badge-pill badge-sm" data-bgcolor="#e7ebf5" data-color="#265ed7"
                                                 style="color: rgb(38, 94, 215); background-color: rgb(231, 235, 245);">{{ $top++ }}</span>
                                            <div class="font-14 weight-600">{{ $customer->username }}</div>
                                            {{-- <div class="font-12 weight-500" data-color="#b2b1b6"
                                                style="color: rgb(178, 177, 182);">Pediatrician</div> --}}
                                        </div>
                                    </div>
                                    <div class="cta flex-shrink-0">
                                        <span class="btn btn-sm btn-outline-primary"
                                            style="font-size: 12px; width: 98px">
                            ...
                            @foreach ($topProductRating as $product)
                                <li class="d-flex align-items-center justify-content-between">
                                    <div class="name-avatar d-flex align-items-center pr-2">
                                        ...
                                        <div class="txt">
                                            <span class="badge badge-pill badge-sm" data-bgcolor="#e7ebf5" data-color="#265ed7"
                                                style="color: rgb(38, 94, 215); background-color: rgb(231, 235, 245);">{{ $topRating++ }}</span>
                                            <div class="weight-600" style="font-size: 13px">{{ $product['product_name'] }}</div>
                                        </div>
                                    </div>
                                    <div class="cta flex-shrink-0">
                                        <span class="btn btn-sm btn-outline-secondary"
                                            style="font-size: 12px; width: 66px">
                                            {{  $product['avg_rating']  }}
                            ...
                            @foreach ($topProductSaled as $product)
                                <li class="d-flex align-items-center justify-content-between">
                                    <div class="name-avatar d-flex align-items-center pr-2">
                                        ...
                                        <div class="txt">
                                            <span class="badge badge-pill badge-sm" data-bgcolor="#e7ebf5" data-color="#265ed7"
                                                style="color: rgb(38, 94, 215); background-color: rgb(231, 235, 245);">{{ $topSaled++ }}</span>
                                            <div class="weight-600" style="font-size: 13px">{{ $product['product_name'] }}</div>
                                        </div>
                                    </div>
                                    <div class="cta flex-shrink-0">
    ...
    <script>
            ...
            const arrRevenue= [<?php if(isset($arrRevenue)) echo '"'.implode('","',  $arrRevenue ).'"' ?>];
            const arrOrder= [<?php if(isset($arrOrder)) echo '"'.implode('","',  $arrOrder ).'"' ?>];
            const countAdmin = <?php if(isset($all_admin)) echo count($all_admin) ?>;
            const countCate = <?php if(isset($all_category)) echo count($all_category) ?>;
            const countStorage = <?php if(isset($all_storage)) echo count($all_storage) ?>;
            const countSlider = <?php if(isset($all_slider)) echo count($all_slider) ?>;
            const countVoucher= <?php if(isset($all_voucher)) echo count($all_voucher) ?>;
            ...
            $('#select_year_filter_order_dashboard').change(function() {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#title_chart_culumn').html('Đồ Thị Biểu Diễn Đơn Hàng MKU FOOD '+year);
                    }
                });
            });
            $('#select_year_filter_revenue_dashboard').change(function() {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#title_chart_area').html('Đồ Thị Biểu Diễn Doanh Thu MKU FOOD '+year);
                    }
                });
            });
